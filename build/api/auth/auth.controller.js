...
export const authCtrl = {
  ...
  async loginP(req, res) {
    ...
    if (result.SCHYR != '') parameters.SCHYR = result.SCHYR;else parameters.SCHYR = "교직원";
    if (result.SCHYR != '') parameters.GRAD_YN = result.GRAD_YN;else parameters.GRAD_YN = "N";
  ...
  async loginOracle(req, res, next) {
    let parameters = {
      "user_id": req.body.user_id,
      "pwd": crypto.createHash('sha512').update(req.body.pwd).digest('base64') // "pwd": req.body.pwd
    ...
    if (!result) throw new BadRequestException("학번 혹은 포털 SW 개인정보 동의를 확인해주세요"); // if (result.PASS !== parameters.pwd) throw new BadRequestException("비밀번호가 틀렸습니다")

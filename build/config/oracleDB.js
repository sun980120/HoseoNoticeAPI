'use strict'; // const oracledb = require('oracledb');

import logger from './logger.js';
import db from './db.js';
import mysql from "mysql"; // oracledb.outFormat = oracledb.OUT_FORMAT_OBJECT;
  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      conn.query(queryData, function (err, db_data) {
        ...
        if (db_data.rows[0] != undefined) resolve(db_data.rows[0]);else reject("학번 혹은 비밀번호를 다시 확인하세요.");
      });
    } catch (error) {
      console.log(error);
      reject(error);
  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      conn.query(queryData, [parameters.user_id, parameters.pwd], function (err, db_data) {
        ...
        if (db_data[0] !== undefined) resolve(db_data[0]);else reject("학번 혹은 비밀번호를 다시 확인하세요."); // else resolve(false)
      });
    } catch (error) {
      console.log(error);
      reject(error);
  ...
  return new Promise(function (resolve, reject) {
    try {
      ...
      conn.query(queryData, [parameters.user_id, parameters.pwd], function (error, db_data) {
        ...
        if (db_data[0] !== undefined) resolve(db_data[0]);else reject("학번 혹은 비밀번호를 다시 확인하세요.");
      });
    } catch (error) {
      reject(error);
    }
